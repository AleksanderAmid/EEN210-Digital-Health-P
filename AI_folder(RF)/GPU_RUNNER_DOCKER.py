...
def test_window_configurations():
    ...
    os.system("nvidia-smi -pm 1")  # Enable persistent mode.
    os.system("nvidia-smi --auto-boost-default=0")

    # Create results directory.
    result_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Model_Result")
    os.makedirs(result_dir, exist_ok=True)
    
    # Headless operation: expect CSV file path as a command-line argument.
    if len(sys.argv) < 2:
        print("Usage: python GPU_RUNNER.py <path_to_training_csv>")
        sys.exit(1)
    csv_path = sys.argv[1]
