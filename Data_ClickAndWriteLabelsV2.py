...
def axel_fix(df):
    ...
    plt.tight_layout()
    return ax_acc, ax_gyro, time


def make_onselect(plot_name, time, df):
    """
    ...
    def _onselect(xmin, xmax):
        xmin = np.array(xmin).item()
        xmax = np.array(xmax).item()
        if xmax < xmin:
            xmin, xmax = xmax, xmin
...
def spanselector(ax_acc, ax_gyro, time, df):
    onselect_acc = make_onselect('accelerometer', time, df)
    onselect_gyro = make_onselect('gyroscope', time, df)
    span_acc = SpanSelector(ax_acc, onselect_acc, 'horizontal', useblit=True)
    span_gyro = SpanSelector(ax_gyro, onselect_gyro, 'horizontal', useblit=True)
    
    ax_button = plt.axes([0.82, 0.01, 0.12, 0.05])
    button = Button(ax_button, 'Spara')
    button.on_clicked(save_data)

    plt.show()  # Blocking call to keep the window open
...
def save_data(event):
    ...
    if df is not None:
        ...
        df.to_csv('LabeledData/labeled_data.csv', index=False)
        print("Data saved to 'LabeledData/labeled_data.csv'")
